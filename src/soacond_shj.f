...
C     Nkf, Mkf, Gcf - same as above, but final values
      ...
      SUBROUTINE soacond(Nki,Mki,Gci,Nkf,Mkf,Gcf,dt,tau_p,Dbk)
...
cdbg      integer numcalls          !number of times cond routine is called
      ...
      DOUBLE PRECISION ACCOM
      DOUBLE PRECISION FC(IBINS)
      DOUBLE PRECISION 
...
C   	  print *, Di
      ...
      tj=2.*pi*Di*mworg(jo)*1.0d-3/(R*temp)
...
Ckpc  Density should be changed due to more species involed. 
            orgmass=0.d0
            do jj=1,iorg
               orgmass=orgmass+Mk(k,srtorg1+jj-1) 
            enddo
...
!     basic properties; can be eliminated if defined in TOMAS prior to this
     ...
     &        + 0.238*accom(i)*Kn + 0.75*accom) ! Fuchs correction, unitless, eqn (14)
!         FC(k) = (1.+Kn)/(1.+2.*Kn*(1.+Kn)/alpha) !S&P eqn 11.35         
...
! Zaveri et al. (2014) specific properties
         ...
         coth(k) = tanh(qk(k))        ! hyperbolic cotangent
         Qk(k) = 3.0 * (qk(k) * coth(k) - 1.0) / qk(k)**2.0 ! ratio of particle concentration to surface concentration at steady state, unitless
         if ((Qk(k).gt.0.99).and.(kc.eq.0.0)) then
            Qk = 1.0;
            kpk(k) = 5.0 * Dbk / Rpk(k)
         else
            kpk(k) = Dbk / Rpk(k) * (qk(k) * coth(k) - 1.0) / (1.0-Qk) ! particle-side mass transfer coefficient, m/s
         endif
         Kgk(k) = 1 / (1/kgk(k) + 1/kpk(k) * (cstar(k)/density/1.0e9))
CCCCC ============================================================================================
!     alia - calculate Kgk
...
!            sK=sK+tk(k)*
!     &         Nkf(k)*(mp)**(1.d0/3.d0)
            sK=sK + 4 * pi * (Dpk(k)/2)**2.0 * Nkf(k) *
     &           Kgk(k) / (dble(boxvol)*1D-6)
         endif
      enddo      
      do k=1,ibins
!         sinkfrac(k)=tk(k)*Nkf(k)*mps(k)**(1.d0/3.d0)/sK
         sinkfrac(k)=4 * pi * (Dpk(k)/2)**2.0 * Nkf(k) * 
     &        Kgk(k) / (dble(boxvol)*1D-6) / sK
      enddo
!      sK=sK*zeta13*tj*R*temp/(mworg(jo)*1.d-3)/(boxvol*1.d-6)
!      sK = 2.d0*pi*dble(Di)*sK/(dble(boxvol)*1D-6)
!      print*,'beta org',beta
!      print*,'CS org',sK
!	  print*,'j=',j
...
C         scalefactor=exp((storg*0.2)/(8.314*temp*1200.*Dpk(k)))  !! kelvin effect?
       scalefactor=exp((storg*0.001*mworg(j))/(8.314*temp*1200.*Dpk(k)))  !! kelvin effect, mw dependent
