...
CCCCC DECLERATION...............................................................................
      ...
      PARAMETER(cdt = 1, kB = 1.38e-23, g = 9.81, R = 8.314)
...
C         kw_gas(j) = StoV*(alpha_w(j)*cmean(j)/4.)/
C     &        (1.+(pi/2.)*(alpha_w(j)*cmean(j)/(4.*sqrt(kei*Dg(j))))) ! Zhang et al., 2014
         IF (VWL .eq. 1) THEN
            kw_gas(j) = kei !2.5e-4
         ELSE
            kw_gas(j) = 0.0
...
C         kw_gas_off(j) = kw_gas(j)*cstarT(j)/wlc(j)/(MWORG(j)*1000.0)/gamma_p
         kw_gas_off(j) = kw_gas(j) * cstarT(j) / wlc_zhang
