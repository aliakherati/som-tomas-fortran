...
CAliA     beta(k)=(1.+Kn)/(1.+2.*Kn*(1.+Kn)/alpha) !S&P eqn 11.35
! AliA ------------------------------------------------------------------------            
         beta(k) = 0.75 * alpha * (1.0+Kn) / (Kn*(1.0+Kn) +
     &        0.238*alpha*Kn + 0.75*alpha) ! Fuchs correction, unitless, eqn (14)
!     AliA ------------------------------------------------------------------------
...
!            sK=sK+tk(k)*
!     &         Nkf(k)*(mp)**(1.d0/3.d0)
             sK=sK+Dpk(k)*Nkf(k)*beta(k)
         endif
      enddo      
      do k=1,ibins
!         sinkfrac(k)=tk(k)*Nkf(k)*mps(k)**(1.d0/3.d0)/sK
         sinkfrac(k)=Dpk(k)*Nkf(k)*beta(k)/sK
      enddo
